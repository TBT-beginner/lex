...
def load_data():
    ...
    return quests_data, skills_data, materials_data, recipes_data

QUESTS, SKILLS, MATERIALS, RECIPES = load_data()
...
def battle_page():
    ...
    return render_template('battle.html', session=session, RECIPES=RECIPES)
...
def submit_answer():
    ...
    if not word_info: return redirect(url_for('battle_page')) # 安全策
    ...
    if user_answer == target_word:
        ...
        if not is_review_quest:
            mistaken_list = session.get('mistaken_words', [])
            if target_word not in mistaken_list:
                mistaken_list.append(target_word)
                session['mistaken_words'] = mistaken_list
                session.modified = True
    ...
    if session['monster_hp'] <= 0:
        if is_review_quest:
            ...
            exp_reward = QUESTS[quest_id]['exp_reward']
            session['exp'] += exp_reward
            loot_table = QUESTS[quest_id].get('loot_table', [])
            loot_message = ""
            if loot_table:
            ...
            session['result_message'] = f"クエスト成功！{exp_reward}EXP獲得！{loot_message}{level_up_message}"
        return redirect(url_for('start_page'))
...
# (reset, skills, learn_skill, workshop, forge_word は変更なし)
@app.route('/reset')
def reset_game():
    session.clear()
    return redirect(url_for('start_page'))
...
@app.route('/workshop')
def workshop_page():
    return render_template('workshop.html', materials=MATERIALS, recipes=RECIPES, session=session)
        ...
        if session.get('charge_power', 0) >= cost:
            ...
            if effect == 'damage':
                ...
                if session['player_hp'] > session.get('max_hp', 50):
                    session['player_hp'] = session.get('max_hp', 50) # 最大HPを超えない
                session['message'] = f"魔導書を開き「{spell_data['name']}」を詠唱！HPが {power} 回復した！"
            
            # 勝敗判定
            if session['monster_hp'] <= 0:
                # (単純化のため、詠唱で倒した場合は通常の勝利処理にリダイレクト)
                # 本来はここでも経験値計算などが必要だが、まずは動作を優先
                quest_id = session.get('current_quest_id')
                session['result_message'] = f"クエスト成功！" # メッセージを簡略化
                return redirect(url_for('start_page'))
